ADSPLACE Core version *3.1.0* is now available from:  <https://github.com/adsplace-project/adsplace/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/adsplace-project/adsplace/issues>


Mandatory Update
==============

ADSPLACE Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.

Users updating from a previous version after Tuesday, May 8, 2019 12:00:00 AM GMT will require a full resync of their local blockchain from either the P2P network or by way of the bootstrap.
...
zADS Staking is here! zADS staking will be activated on the morning of the 8th of May 2019. With the release of zADS staking, there are effectively 2 versions of zADS, zADS minted on the 3.0.6 ADSPLACE wallet or lower, and zADS minted on ADSPLACE wallet version or higher. New features in this release will require the use of zADS v2, zADS minted on this wallet release 3.1.0 or later. If you currently hold zADS v1 and wish to take advantage of zADS staking and deterministic zADS, you will need to spend the zADS v1 to yourself and remint zADS v2.
Note: To find your zADS version, click the privacy tab, then the zADS Control button then expand the arrows next to the desired denomination.


### Deterministic zADS Seed Keys

zADS is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zADS outside of the wallet that the zADS had been minted on. zADS can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zADS minting

zADS minting now only requires 1 further mint (down from 2) to mature. zADS mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zADS Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zADS denomination has been spent or not.



ADS/zADS Staking and Masternode Rewards
--------------

### ADS, zADS and Masternode Payment Schedule

To encourage the use of zADS and increase the ADSPLACE zerocoin anonymity set, the ADSPLACE payment schedule has been changed to the following:

If a user staking zADS wins the reward for their block, the following zADS reward will be: 
- 3 zADS (3 x 1 denominations) rewarded to the staker, 2 ADS rewarded to the masternode owner and 1 ADS available for the budget. This is a total block reward of 6 ADS, up from 5.

If a user staking ADS wins the reward, the following amounts will be rewarded: 
- 2 ADS to the ADS staker, 3 ADS to the Masternode owner and 1 ADS available for the budget. This is a total block reward of 6 ADS, up from 5.


### Return change to sender when minting zADS

Previously, zADS minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most ADS and return the change from a zADS mint to that address.
...
The visual layout of the ADSPLACE Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zADS minting feature. This is enabled by default and the enablezeromint=0 setting in the adsplace.conf file will overwrite the GUI option.
-	The percentage of autominted zADS can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular ADS.
...
### Summary
 ...
 The ADSPLACE wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the adsplace.conf file, found in the ADSPLACE data directory.
- backuppath = <directory / full path>
- zADSbackuppath = <directory / full path>
...
* If zADS backup, auto generated name is `wallet-autozADSbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
Backing up 4 times will result as shown below
                ...
                date/time
    backup #1 - 2019-04-20-00-04-00  
    backup #2 - 2019-04-21-04-20-00  
    backup #3 - 2019-04-22-00-20-04  
    backup #4 - 2019-04-23-20-04-00  
    
    1.
        /<mynewdir>/
            wallet.dat-2019-04-20-00-04-00
    2.
        /<mynewdir>/
            wallet.dat-2019-04-20-00-04-00
            wallet.dat-2019-04-21-04-20-00
    3.
        /<mynewdir>/
            wallet.dat-2019-04-22-00-20-04
            wallet.dat-2019-04-21-04-20-00
    4.
        /<mynewdir>/
            wallet.dat-2019-04-22-00-20-04
            wallet.dat-2019-04-23-20-04-00
            


ADSPLACE Daemon & Client (RPC Changes)
--------------
...
This allows running adsplaced without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the ADSPLACE support group, which often times sees users say "I didn't spend that zADS". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the ADSPLACE masternodes, the monthly budget available to be utilised has been increased to 42,000 ADS / month. This ADS only has the opportunity to be raised once per month (paid to winning proposals) with any unused ADS not created by the blockchain.

### Budget Finalization Fee

The ADSPLACE finalization fee for successful proposals has now been reduced, this fee is now 5 ADS down from 50 ADS. The total fee outlay for a successful proposal is now a total of 55 ADS.
...
-	Enforced v1 zADS spends to require a security level of 100
-	Updates to zADS spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the ADSPLACE-Qt masternodes tab
-	Updated ADSPLACE-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and ADSPLACE's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zADS Control dialog in the QT wallet, a user is able to see zADS marked as version 1 or 2.

zPoS (zADS staking)
--------------

Once a zADS has over 200 confirmations it becomes available to stake. Staking zADS will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zADS. So for example if a 1,000 zADS denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zADSs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within ADSPLACE's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zADS, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zADS that can be recalculated at any time using the seed. Deterministic zADS allows for users to backup all of their future zADS by simply recording their seed and keeping it in a safe place (similar to backing up a private key for ADS). The zADS seed needs to remain in the wallet in order to spend the zADS after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zADS data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

ADSPLACE's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which ADSPLACE used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to ADSPLACE using the bug bounty program. The modulus is now correctly imported and ADSPLACE's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zADS v2: zPoS dzADS ezADS external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zADS balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zADS status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zADS Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault adsplace-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zADS spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zADS. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zADS. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/adsplace-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
