...
//
// Bootup the Masternode, look for a 1000 ADSPLACE input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (!fMasterNode) {
		LogPrintf("CActiveMasternode::ManageStatus() - it is not master node\n");
		return;
	}

    // if (fDebug) 
		LogPrintf("CActiveMasternode::ManageStatus() - Begin with status: %s\n", GetStatus());
    ...
    if (status == ACTIVE_MASTERNODE_INITIAL) {
        ...
        if (pmn != NULL) {
			LogPrintf("CActiveMasternode::ManageStatus() last chanse %s\n", GetStatus());
            pmn->Check();
            if (pmn->IsEnabled() && pmn->protocolVersion == PROTOCOL_VERSION) EnableHotColdMasterNode(pmn->vin, pmn->addr);
        }else{
			LogPrintf("CActiveMasternode::ManageStatus() something wrong %s\n", GetStatus());
		}
    }
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if(!CMasternodeBroadcast::CheckDefaultPort(strMasterNodeAddr, errorMessage, "CActiveMasternode::ManageStatus()")){
			LogPrintf("CActiveMasternode::ManageStatus() port is not default: %s\n", service.ToString());
            return;
		}
		...
		LogPrintf("CActiveMasternode::ManageStatus() - node already started"); 
	}
...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex)
{
	...
	// wait for reindex and/or import to finish
	if (fImporting || fReindex) return false;
LogPrintf("CActiveMasternode::GetMasterNodeVin input stack: %s\n", vin.ToString());
    // Find possible candidates
    TRY_LOCK(pwalletMain->cs_wallet, fWallet);
	
    if (!fWallet) return false;
LogPrintf("CActiveMasternode::GetMasterNodeVin fWallet ? is true\n");
    vector<COutput> possibleCoins = SelectCoinsMasternode();
    COutput* selectedOutput;

    // Find the vin
	//test only!!!!
	// std::string strTxHash = "74994f67b2fa03405e44542738a6d89e7d259aa8e48066047433f5ba265967fe";
    if (!strTxHash.empty()) {
        // Let's find it
		...
		LogPrintf("CActiveMasternode::GetMasterNodeVin possibleCoins.size() = %s\n", possibleCoins.size());
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
	...
	LogPrintf("CActiveMasternode::SelectCoinsMasternode() vCoins size: %s \n", vCoins.size());
    // Filter
    BOOST_FOREACH (const COutput& out, vCoins) {
		LogPrintf("CActiveMasternode::SelectCoinsMasternode() item: %s \n", out.tx->vout[out.i].ToString());
