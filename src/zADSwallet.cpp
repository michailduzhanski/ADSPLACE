...
CzADSWallet::CzADSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zADS seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZADSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZADSSeed_deprecated()) {
                    LogPrintf("%s: Updated zADS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zADS seed\n", __func__);
                }
            }
        }
    }
...
bool CzADSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZADSCount(nCountLastUsed);
    else if (!walletdb.ReadZADSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzADSWallet::Lock()
{
    seedMaster = 0;
}

void CzADSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzADSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZADS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzADSWallet::LoadMintPoolFromDB()
{
...
void CzADSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzADSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzADSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zADSTracker->HasPubcoinHash(pMint.first)) {
...
bool CzADSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZADS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zADSTracker which also adds to database
    pwalletMain->zADSTracker->Add(dMint, true);
...
void CzADSWallet::SeedToZADS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzADSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzADSWallet::GenerateDeterministicZADS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzADSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzADSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
