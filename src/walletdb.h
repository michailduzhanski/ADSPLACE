...
public:
    ...
    bool WriteZADSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZADSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZADSSeed_deprecated(uint256& seed);
    bool EraseZADSSeed();
    bool EraseZADSSeed_deprecated();

    bool WriteZADSCount(const uint32_t& nCount);
    bool ReadZADSCount(uint32_t& nCount);
