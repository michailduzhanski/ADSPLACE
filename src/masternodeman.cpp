...
{
    LOCK(cs);
	uint256 selfHashPubKey = Hash(pubKeyMasternode.begin(), pubKeyMasternode.end());
LogPrintf("Start find: %s in %s\n", selfHashPubKey.ToString(), vMasternodes.size());
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
		uint256 listHashPubKey = Hash(mn.pubKeyMasternode.begin(), mn.pubKeyMasternode.end());
		
		LogPrintf("masternode: %s == %s", listHashPubKey.ToString(), selfHashPubKey.ToString());
...
LogPrintf("Start ProcessMessage with command: %s\n",strCommand);
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 ADSPLACE tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 ADS tx -> 1 confirmation
