...
#ifndef ADSPLACE_ZADSWALLET_H
#define ADSPLACE_ZADSWALLET_H
...
public:
    CzADSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZADS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZADS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ADSPLACE_ZADSWALLET_H
