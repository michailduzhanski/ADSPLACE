...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "adsplace.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "adsplaced.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ADS and zADS money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ADS/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ADS/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, adsplace, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ADS/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ADSPLACE specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ADSstake=<n>", strprintf(_("Enable or disable staking functionality for ADS inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zADSstake=<n>", strprintf(_("Enable or disable staking functionality for zADS inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzADS=<n>", strprintf(_("Enable automatic wallet backups triggered after each zADS minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zADSbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zADS backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeadsplaceamount=<n>     " + strprintf(_("Keep N ADS anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The ADSPLACE Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ADSPLACE is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ADSPLACE Core is shutting down."));
...
#endif
    // Make sure only a single ADSPLACE process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ADSPLACE Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ADSPLACE version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //ADSPLACE: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //ADSPLACE specific: zerocoin and spork DB's
                ...
                // ADSPLACE: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZADSMinted();
                        RecalculateZADSSpent();
                    }
                    RecalculateADSSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // ADSPLACE: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ADSPLACE Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ADSPLACE Core to complete") << "\n";
        ...
        zwalletMain = new CzADSWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zADSWallet
        uiInterface.InitMessage(_("Syncing zADS wallet..."));

        bool fEnableZADSBackups = GetBoolArg("-backupzADS", true);
        pwalletMain->setZADSAutoBackups(fEnableZADSBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zADSTracker->Init();
...
//    }
//
//    nAnonymizeAdsplaceAmount = GetArg("-anonymizeadsplaceamount", 0);
//    if (nAnonymizeAdsplaceAmount > 999999) nAnonymizeAdsplaceAmount = 999999;
//    if (nAnonymizeAdsplaceAmount < 2) nAnonymizeAdsplaceAmount = 2;
    ...
    LogPrintf("Anonymize ADSPLACE Amount %d\n", nAnonymizeAdsplaceAmount);
    ...
    /* Denominations
       ...
       For example:
       1ADS+1000 == (.1ADS+100)*10
       10ADS+10000 == (1ADS+1000)*10
    */
