...
#include "ui_zADScontroldialog.h"
...
std::set<std::string> ZADSControlDialog::setSelectedMints;
std::set<CMintMeta> ZADSControlDialog::setMints;

ZADSControlDialog::ZADSControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZADSControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZADSControlDialog::~ZADSControlDialog()
{
    delete ui;
}

void ZADSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZADSControlDialog::updateList()
{
...
void ZADSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZADSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZADS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZADSControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZADSControlDialog::GetSelectedMints()
{
...
void ZADSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
