...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ADS);
    ...
    if (!settings.contains("nAnonymizeAdsplaceAmount"))
        settings.setValue("nAnonymizeAdsplaceAmount", 1000);

    nAnonymizeAdsplaceAmount = settings.value("nAnonymizeAdsplaceAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAdsplaceAmount"))
        SoftSetArg("-anonymizeadsplaceamount", settings.value("nAnonymizeAdsplaceAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in adsplace.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAdsplaceAmount:
            return QVariant(nAnonymizeAdsplaceAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAdsplaceAmount:
            nAnonymizeAdsplaceAmount = value.toInt();
            settings.setValue("nAnonymizeAdsplaceAmount", nAnonymizeAdsplaceAmount);
            emit anonymizeAdsplaceAmountChanged(nAnonymizeAdsplaceAmount);
            break;
        case CoinControlFeatures:
